...
# Respuesta para que quede bonito
data2 = log2(data)
...
# ¿Qué es un boxplot?
# Respuesta Es un gráfico donde se observa la media de los datos, el rango intercuartílico, los datos extremos...
# Respuesta las=2 es para que los nombres de las columnas nos salgan en vertical
...
# Clasifica los datos por la similitud, si es correcta la separación porque separa wildtipe de knockout
...
# Hemos seleccionado las columnas de 1-3 para WT y de 4-6 para KO
wt <- data[,1:3]
ko <- data[,4:6]
class(wt)
head(wt)
...
dim(filtered)
head(filtered)
# 426 genes cumplen los criterios
...
# La sobreexpresión es ko y los reprimidos son wt. LOs negros son los que no pasan el filtro
